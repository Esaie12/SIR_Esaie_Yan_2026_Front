...
export class MessageEditComponent {
  ...
  submit() {
    ...
    const { typeDestinataire, ...rest } = this.form;
    ...
    const payload = {
      ...
      userId: this.form.typeDestinataire === 'user'
        ? this.form.userId ?? null
        : null,

      groupeId: this.form.typeDestinataire === 'group'
        ? this.form.groupeId ?? null
        : null,
    ...
    this.messageService.update(this.messageId, payload).subscribe({
      ...
      error: err => {
        console.error(err);
        this.error = 'Une erreur est survenue lors de la mise à jour du message.';
      }
    });
  ...
  fetchClients() {
    this.customerService.getUserMessages(1).pipe(
      map((res: any) => res.data)
